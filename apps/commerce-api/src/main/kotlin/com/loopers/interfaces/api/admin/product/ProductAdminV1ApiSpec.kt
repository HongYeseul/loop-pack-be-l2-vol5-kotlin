...
interface ProductAdminV1ApiSpec {
    ...
    fun getProducts(page: Int?, size: Int?): ApiResponse<PageResponse<ProductAdminV1Dto.ProductResponse>>

    @Operation(summary = "상품 생성", description = "삭제되지 않은 브랜드에만 만들 수 있습니다 (P-05).")
    fun createProduct(request: ProductAdminV1Dto.CreateRequest): ApiResponse<ProductAdminV1Dto.ProductResponse>

    @Operation(summary = "상품 상세", description = "삭제된 상품도 조회합니다. 재고 수량과 삭제 시각을 포함합니다.")
    fun getProduct(productId: Long): ApiResponse<ProductAdminV1Dto.ProductResponse>

    @Operation(summary = "상품 수정", description = "이름과 가격만 바꿉니다. 브랜드는 바꿀 수 없습니다 (P-05).")
    fun updateProduct(productId: Long, request: ProductAdminV1Dto.UpdateRequest): ApiResponse<ProductAdminV1Dto.ProductResponse>
    ...
    fun changeStock(productId: Long, request: ProductAdminV1Dto.StockRequest): ApiResponse<ProductAdminV1Dto.ProductResponse>

    @Operation(
        summary = "판매 상태 설정",
        description = "ON_SALE · SUSPENDED · DISCONTINUED 중 하나입니다. 단종은 되돌릴 수 없습니다 (P-36).",
    )
    fun changeStatus(productId: Long, request: ProductAdminV1Dto.StatusRequest): ApiResponse<ProductAdminV1Dto.ProductResponse>
}
